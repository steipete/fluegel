...
The local gate tests the package and builds both Apple Silicon and Intel slices. `scripts/build-app.sh` builds only the host architecture; add `--universal` for both. The packaging script makes a release build, creates `dist/Fluegel.app`, copies the CLI to `dist/fluegel`, writes the app metadata, and ad-hoc signs the bundle.
...
The package has no external SwiftPM dependencies. CI uses the runner's existing tools without third-party actions. The separate release workflow uses pinned GitHub artifact actions. It does not launch the GUI app or request privacy permissions; launch and bridge checks still require local verification with the appropriate signing identity.

## Releases

See [Releasing](RELEASING.md) for the signed, notarized GitHub release workflow. `Sources/FluegelCore/Version.swift` is the shared CLI and app version source.
